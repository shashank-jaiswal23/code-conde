...
const Header = ({
  ...
  selectedLocale,
  state,
  sections = [],  // Default to empty array if undefined
  selectedSection,
  setState,
}) => {
  ...
  useEffect(() => {
    if (mobileExpanded && popup.current) {
      popup.current.focus();
      disableBodyScroll(popup.current); // Prevent body scroll when mobile menu is open
    } else {
      enableBodyScroll(popup.current); // Allow body scroll when mobile menu is closed
    }
  }, [mobileExpanded]);

  useEffect(() => {
    const closeHeaderWithEsc = (e) => {
      if (e.keyCode === 27) { // ESC key code
        setMobileExpanded(false);
      }
    };

    window.addEventListener('keyup', closeHeaderWithEsc);

    return () => {
      window.removeEventListener('keyup', closeHeaderWithEsc);
    };
  }, []);
    ...
    >
      <header className={`state-${state}`}>
        ...
        <div id="to-conde" className="no-margin curp psf px1 py1">
          <a target="_blank" rel="noopener noreferrer" href="https://condenast.com">
        ...
        >
          {sections.map((s, i) => {
            ...
            return (
              <div key={s.id || i} className={`small active-${active}`}>
                <button
                  tabIndex={`${state === 'content' ? '1' : '-1'}`}
                  onClick={navigate}
                >
                  <p>
                    {i + 1}. {s.fields?.headerTitle || 'Untitled Section'}
                  </p>
                </button>
              </div>
            );
              ...
              {mobileExpanded && (
                <FocusTrap>
                  <div>
                    ...
                    <div onClick={toggleMobileHeader} id="logo" className="curp px1 py1">
                      <h5>The Condé Code</h5>
                    </div>
                    ...
                    <div className="mobile-nav-items px1 py1 mb2">
                      {sections.map((s, i) => {
                        ...
                        return (
                          <button key={s.id || i} onClick={navigate} className="db small">
                            <h5>
                              {i + 1} <span className="pr1"> </span> {s.fields?.headerTitle || 'Untitled Section'}
                            </h5>
                          </button>
                        );
                      })}
                    ...
                    <section className="footer-links px1">
                      <div className="mb1">
                        {footerLinks.map((link, i) => (
                          <a className="nav-link db" key={i} href={link.link}>
                            {link.title}
                          </a>
                        ))}
                      </div>
