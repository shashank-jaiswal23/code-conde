...
import { jsonModifier, ImageModifier, crousal } from './functions'
const listLocales = 'i18n/locales'
export const getEntry = async (locale) => {
  try {
    const dataPath = `${process.env.HOST}${process.env.CONTENT}?populate=deep,7&locale=${locale}`
    const dataEngPath = `${process.env.HOST}${process.env.CONTENT}?populate=deep,7`

    const locales = await axios
    .get(`${process.env.HOST}${process.env.CONTENT}?populate=localizations`)
    .then((res) => {
        ...
        // Apply jsonModifier to res.data
        const modifiedData = jsonModifier(res.data);
    
        // Ensure modifiedData is an array and has at least one element
        if (!Array.isArray(modifiedData) || modifiedData.length === 0) {
            throw new Error("jsonModifier did not return a valid array or is empty");
        }
    
        // Access the 'localizations' property in the first item
        const localizations = modifiedData[0]?.localizations;
    
        // Check if localizations exists
        if (!localizations) {
            throw new Error("The first item in the data does not contain 'localizations'");
        }
    
        // Return the modified localizations
        return jsonModifier(localizations);
    })
    .catch((error) => {
        console.error("An error occurred:", error);
        // Optionally handle the error or show a fallback
    });
    
    // const authPath = `${process.env.HOST}auth/local`
    // console.log(authPath, 9)
    // const credentials = {
    ...
    .get(dataPath)
    .then((resp) => {
        // Check if resp.data is valid
        if (!resp || !resp.data) {
            throw new Error("Response or response data is undefined or null");
        }
    
        // Apply jsonModifier to resp.data
        const modifiedData = jsonModifier(resp.data);
    
        // Ensure modifiedData is not undefined
        if (!modifiedData) {
            throw new Error("jsonModifier returned an invalid or undefined result");
        }
    ...
    .catch((error) => {
        console.error("An error occurred:", error);
        // Optionally handle the error or return fallback data
    });
    ...
    const localesList = await axios
      .get(`${process.env.HOST}${listLocales}`)
      .then((resp) => resp.data)
      ...
      const output = data.map((item, index) => {
        ...
        return {
          ...
          fields: {
            ...
            backgroundMedia: fetchCoverMedia(ImageModifier(item.background_media)),
          },
        }
      })
      return output
    ...
    const constructedFooterVal = (data, sectionIndex) => {
      const output = data.map((item, index) => {
        ...
        return {
          ...
          fields: {
            title,
            media: fetchCoverMedia(ImageModifier(item.Media)),
          },
        }
      })
      return output
    ...
    const constructedCarouselVal = (data, sectionIndex) => {
      const output = data.map((item, index) => {
        ...
        const composeArr = crousal(item.Images).map((sub) => fetchCoverMedia((sub)))
        return {
          sys: {
            contentType: {
              sys: {
      ...
      const output = data.map((item, index) => {
        ...
        return {
          ...
          fields: {
            ...
            media1: fetchCoverMedia(ImageModifier(item.Media_1)),
            media2: fetchCoverMedia(ImageModifier(item.Media_2)),
          },
        }
      })
      return output
    ...
    const customizeDataArray = ({ type, record }) => {
      if (record && (record.length > 0 || record.data.length > 0)) {
        if (type === 'landingSlideshow') {
          // don't need image modifier as crousel method handles mapping
          ...
          const composeArr = record.map((item, index) => {
            ...
            // console.log(JSON.stringify(item.blocks_component, null, 2), 787877787)
            return {
              fields: {
                title,
                headerTitle,
                coverMedia: fetchCoverMedia(ImageModifier(item.Cover_Media)),
                longText,
    ...
    // console.log(entryval[0].components[0].content_section, 520)
    const requestData = {
      fields: {
        title,
        landingSlideshow: customizeDataArray({
          type: 'landingSlideshow',
          record: crousal(entryval[0].components[0].Landing_Slideshow),
        }),
    ...
    function filterLocals(localesList, englishData) {
      let validLocals = locales.map(item => item.locale);
      return localesList.filter((item) => item.code === "en" || validLocals.includes(item.code))
    }

    const customizeLocale = filterLocals(localesList, entryval).map((item) => {
      return {
        code: item.code,
        name: item.name,
